...
// Simple alternative to JSONB arrays - easier to query and maintain.
// ─────────────────────────────────────────────────────────────────────────────
export const CharacterImage = {
  /** Get all images for a character, newest first */
  ...
  /** Add a new image for a character */
  async add(characterId, { url, label, seed, poseId, emotionEntry, paramsSnapshot, generationType = 'sprite', jobId }) {
    const { data, error } = await supabase
      .from('character_images')
      .insert({
  ...
  /** Get images for multiple characters (for gallery pages) */
  async forCharacters(characterIds) {
